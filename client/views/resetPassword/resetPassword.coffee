...
Template.enrollPassword.helpers
  error: -> Session.get('entryError')

  logo: ->
    AccountsEntry.settings.logo

Template.enrollPassword.events
  ...
  'submit #resetPassword': (event) ->
    ...
    Accounts.resetPassword Session.get('enrollToken'), password, (error) ->
      if error
        Session.set('entryError', (error.reason || "Unknown error"))
      else
        Session.set('enrollToken', null)
        Router.go AccountsEntry.settings.dashboardRoute
